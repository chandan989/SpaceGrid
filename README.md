...
Space Grid is a decentralized physical infrastructure network (DePIN) and location-based protocol deployed on the **Creditcoin EVM** — a purpose-built Layer 1 blockchain for infrastructure and financial inclusion. Space Grid gamifies the mapping and verification of geospatial data, allowing operators ("Grid Runners") to translate physical movement into cryptographic trails and claim territories as persistent, tokenized assets.

> **Built on Creditcoin** — the world's leading infrastructure chain, with over 3 million on-chain credit transactions recorded and an EVM-compatible mainnet live since August 2024.
...
Rather than resetting after a session, Space Grid leverages Creditcoin's EVM to permanently record captured territories — turning localized spatial control into verifiable assets stored directly on-chain.
